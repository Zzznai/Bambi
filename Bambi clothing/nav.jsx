...
function Navbar({ route }) {
  const { user, isLoggedIn, isAdmin, logout } = window.useAuth();
      ...
      <div className="nav__inner">
        ...
        {isLoggedIn ? (
          <div className="nav__user">
            <div className="nav__avatar" title={user.username}>{user.username[0].toUpperCase()}</div>
            <div style={{ display: "flex", flexDirection: "column", lineHeight: 1.1 }}>
              <span style={{ fontWeight: 700, fontSize: 13 }}>@{user.username}</span>
              <span className="muted" style={{ fontSize: 11 }}>{user.role}</span>
            </div>
            <button className="btn btn--ghost btn--sm" onClick={() => { logout(); window.navigate("/shop"); }}>
