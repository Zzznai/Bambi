...
import { useApi, useToast, useConfirm, PageWrap, ListingCard, Empty } from './components.jsx';
...
export function MyListingsPage() {
  ...
  const load = () => {
    setLoading(true);
    apiCall(() => api.listMyListings(1, 200))
  ...
  const handleDelete = async (l) => {
    ...
    if (!ok) return;
    const res = await apiCall(() => api.deleteListing(l.id));
    if (res) {
      toast.ok("Deleted.");
      load();
    }
        ...
        <div className="grid-listings">
          {filtered.map(l =>
            <div key={l.id} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <ListingCard listing={l} onClick={() => navigate("/listing/" + l.id)} />
