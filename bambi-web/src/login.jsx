...
export function LoginPage() {
  ...
  // If the user is already logged in when landing on /login (e.g. came back
  // here directly), bounce them to wherever they belong — admin → /admin,
  // everyone else → /shop. Skipped during the login submit itself because the
  // handler does its own role-aware navigate first.
  React.useEffect(() => {
    if (auth.isLoggedIn) {
      navigate(auth.isAdmin ? "/admin" : "/shop");
    }
